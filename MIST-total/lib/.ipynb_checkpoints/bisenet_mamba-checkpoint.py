...
class DetailBranch(nn.Module):
    ...
    def forward(self, x):
        ...
        feat = self.mamba1(feat)
        feat = feat.permute(0, 2, 1).view(B, C, H, W)
        print("detailed breach feat:", feat.shape)
        feat = self.S2(feat)
        B, C, H, W = feat.shape
        feat = feat.view(B, C, H * W).permute(0, 2, 1)
        feat = self.mamba2(feat)
        feat = feat.permute(0, 2, 1).view(B, C, H, W)
        print("detailed breach feat2:", feat.shape)
        feat = self.S3(feat)
        B, C, H, W = feat.shape
        feat = feat.view(B, C, H * W).permute(0, 2, 1)
        feat = self.mamba3(feat)
        feat = feat.permute(0, 2, 1).view(B, C, H, W)
...
if __name__ == "__main__":
    ...
    model = BiSeNetV2(n_classes=5,aux_mode='train').to(device)
    model.eval()
    outs = model(x)
    # print(outs.shape)
    for out in outs:
