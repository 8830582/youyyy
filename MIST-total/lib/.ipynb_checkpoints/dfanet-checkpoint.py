...
class DFA_Encoder(nn.Module):
    ...
    def __init__(self,channel_cfg):
        ...
        self.conv1=nn.Sequential(nn.Conv2d(in_channels=1,out_channels=8,kernel_size=3,stride=2,padding=1,bias=False),
                                nn.BatchNorm2d(num_features=8),
                                nn.ReLU())
...
if __name__=='__main__':
    ...
    # MIST-total/lib/pretrained_pth/dfanet/dfa-backbone.pth
    torch.save(bk.state_dict(),'./pretrained_pth/dfanet/dfa-backbone.pth')
    bk=bk.to(device)

    dfa=DFANet(ch_cfg,64,5)
    dfa=load_backbone(dfa,'./pretrained_pth/dfanet/dfa-backbone.pth')
