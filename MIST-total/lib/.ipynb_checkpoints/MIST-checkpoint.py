...
class Attention(nn.Module):
    def __init__(self,
        ...
        # print('embed_dim,num_heads:',channels,self.num_heads)
...
class ChannelAttentionCBAM(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)#对输入特征图进行全局平均池化，将每个通道的特征图压缩为1x1大小。
        self.max_pool = nn.AdaptiveMaxPool2d(1)#对输入特征图进行全局最大池化，将每个通道的特征图压缩为1x1大小。
    ...
    def forward(self, x):
        ...
        out = avg_out + max_out
        return self.sigmoid(out)#此为通道注意力权重
...
class DS_out(nn.Module):#需要特征上采样和逐层精细处理的任务
...
class CAM(nn.Module):
    def __init__(self, args):
        ...
        #tiny
        # filters = [64, 128, 256, 512, 512*2, 512, 256, 128, 64]
        ...
        # print('filters[4], filters[5], att_heads[5], dpr[5]:',filters[4], filters[5], att_heads[5], dpr[5])
    ...
    def forward(self, skip1, skip2, skip3, skip4):
        ...
        #print(x.shape)
        # Multi-scale input
        # print('x, skip4:',x.shape,skip4.shape)
