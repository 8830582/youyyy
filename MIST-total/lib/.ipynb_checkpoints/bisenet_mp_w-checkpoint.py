#bisenetv3带有一层segment——mamba两块与walvet交叉融合
import torch
import torch.nn as nn
import torch.nn.functional as F
import torch.utils.model_zoo as modelzoo

from lib.change_mamba2 import vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2
...
class BiSeNet_total(nn.Module):

    def __init__(self, n_classes, aux_mode='train'):
        super(BiSeNet_total, self).__init__()
...
#segment branch
        ...
        self.mamba1=vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False,num_channels = 32,num_H = 64,num_W = 64)
        self.mamba2 =vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False,num_channels = 64,num_H = 32,num_W = 32)
        self.mamba3 =vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False,num_channels = 128,num_H = 16,num_W = 16)
        self.cbr1 = ConvBNReLU(32,32,2,2,0)
        self.cbr2 = ConvBNReLU(64,64,2,2,0)
        self.cbr3 = ConvBNReLU(128,128,2,2,0)
    def forward(self, x):
        x = x.repeat(1, 3, 1, 1)
        size = x.size()[2:]
#detail branch
...
#example##########fuse1################
        ...
        feat2_aux=self.DWT1(d_feat1,feat2)
        feat2_aux = self.cbr1(feat2_aux)
        feat2 = feat2+feat2_aux
        ############mamba2################
        feat3=self.S3(feat2)
        # print('feat3:',feat3.shape)
        feat3=self.mamba2(feat3)
        # print('d_feat2:',d_feat2.shape)
        feat3_aux=self.DWT2(d_feat2,feat3)
        feat3_aux = self.cbr2(feat3_aux)
        feat3 = feat3+feat3_aux
        ################mamba3#######################
        feat4=self.S4(feat3)
        feat4=self.mamba3(feat4)
        feat4_aux=self.DWT3(d_feat3,feat4)
        feat4_aux = self.cbr3(feat4_aux)
        feat4 = feat4+feat4_aux
        ##########
        feat5_4 = self.S5_4(feat4)
...
#fused branch
        ...
        if self.aux_mode == 'train':
            ...
            return logits_aux4, logits_aux5_4,logits, 
            # return logits
