...
#         return 0, 0, 0, 0
def calculate_metric_percase(pred, gt):
     # 将预测和真实标签二值化：大于0的部分设为1（表示正类），其他设为0
    pred[pred > 0] = 1
    gt[gt > 0] = 1
    # print(pred.shape, gt.shape)
    # 如果预测和真实标签中都有正类（即至少有部分被分为前景）
    if pred.sum() > 0 and gt.sum()>0:
...
def test_single_volume_1out(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1,
                       class_names=None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if class_names == None:
        mask_labels = np.arange(1, classes)
...
def test_single_volume1(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1,
                       class_names=None,test_save =None):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if class_names == None:
        mask_labels = np.arange(1, classes)
    else:
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                for i in range(1, classes):
                    preds_o.append(pred == i)
                # print('preds_o:',preds_o)
                # print(np.array(image[ind, :, :]).shape)
                # print(np.array(masks).shape)
                # print(np.array(preds_o).shape)
                ...
                # print(image[ind, :, :].dtype,masks.dtype)
                if not os.path.exists(test_save_path):
                    os.makedirs(test_save_path)
                if test_save is not None:
                    fig_gt = overlay_masks(np.array(image[ind, :, :]), np.array(masks).transpose(1, 2, 0), labels=mask_labels, colors=cmap,return_type="mpl")
                    fig_pred = overlay_masks(np.array(image[ind, :, :]), np.array(preds_o).transpose(1, 2, 0), labels=mask_labels, colors=cmap,return_type="mpl")
                    # Do with that image whatever you want to do.
                    fig_gt.savefig(test_save_path + '/' + case + '_' + str(ind) + '_gt.png', bbox_inches="tight", dpi=60)
                    fig_pred.savefig(test_save_path + '/' + case + '_' + str(ind) + '_pred.png', bbox_inches="tight",dpi=60)
                    plt.close(fig_gt)
                    plt.close(fig_pred)
                prediction[ind] = pred
    else:
...
#获得每一个类的计算结果
    for i in range(1, classes):
        # Synapse (89, 512, 512) (89, 512, 512)

        # print('calculate_metric_percase',calculate_metric_percase(prediction == i, label == i))
        metric_list.append(calculate_metric_percase(prediction == i, label == i))
        # print(metric_list)
...
def val_single_volume(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    ...
    metric_list = []
    for i in range(1, classes):
        print('calculate_dice_percase:',calculate_dice_percase(prediction == i, label == i))
...
def val_single_volume_1out_chaos_iou(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    with torch.no_grad():
        image,label = image.cuda(),label.cuda()
        p1 = net(image)
        outputs = p1
        out = torch.argmax(outputs,dim=1)
        miou=calculate_iou_percase(out,label)
        


    return miou
    
# def val_single_volume_1out_chaos_dice(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
...
#         metric_list.append(calculate_dice_percase(prediction == i, label == i))
#     return metric_list
# def val_single_volume_1out_chaos_dice(pred, label,classes,patch_size=[256, 256],):
...
def dice_coefficient_per_class(pred, target, num_classes, epsilon=1e-6):
    """
    计算多类分割任务中每个类别的 Dice 系数。
    :param pred: 预测的分割结果（PyTorch 张量）
    :param target: 真实的分割结果（PyTorch 张量）
    :param num_classes: 类别数量
    :param epsilon: 防止除以零的小常数
    :return: 每个类别的 Dice 系数列表
    """
    dice_scores = []
...
def dice_coefficient(pred, target, num_classes, epsilon=1e-6):
    ...
    for class_idx in range(num_classes):
        ...
        dice = (2. * intersection + epsilon) / (union + epsilon)
        dice_scores.append(dice.item())
    return dice_scores[-4:]
def calculate_iou_per_class(pred, target, num_classes):
    """
    计算每个类的 IoU
    :param pred: 模型预测的分割结果，形状为 (H, W) 或 (N, H, W)，其中 N 是批次大小
    :param target: 真实标签，形状为 (H, W) 或 (N, H, W)
    :param num_classes: 类别总数
    :return: 每个类的 IoU，形状为 (num_classes,)
    """
    pred = torch.argmax(pred, dim=1)
    ious = []
...
#     """
#     计算每个类别的 Dice 系数。
#     :param pred: 模型预测的张量，形状为 [batch_size, C, H, W]
#     :param target: 真实标签的张量，形状为 [batch_size, 1, H, W]
#     :param epsilon: 防止除零的常数
#     :return: 每个类别的平均 Dice 系数
#     """
#     # 去掉 target 的多余维度
#     target = target.squeeze(1)  # target 形状从 [batch_size, 1, H, W] 变为 [batch_size, H, W]

#     # 将预测通过 softmax 转化为概率
#     pred = F.softmax(pred, dim=1)

#     # 将 target 转换为 one-hot 编码，形状为 [batch_size, C, H, W]
...
#     dice = (2. * intersection + epsilon) / (union + epsilon)
#     muticlass_dice = torch.mean(dice, dim=0)
#     avg_dice = torch.mean(muticlass_dice[-4:], dim=0)
#     # 返回每个类别的 Dice 系数均值
#     return muticlass_dice[-4:],avg_dice # 返回的是每个类别的平均 Dice 系数,总的平均Dice系数
